...
class GHPath:
    ...
    branch: str
    path: str = ""
    _info: list[dict[str, str]] = dataclasses.field(hash=False, default_factory=list, repr=False)
    ...
    def open(self, mode: Literal["r", "rb"] = 'r') -> io.IOBase:
        val: io.StringIO = open_url(f'https://raw.githubusercontent.com/{self.repo}/{self.branch}/{self.path}')
        if 'b' in mode:
            return io.BytesIO(val.read().encode('utf-8'))
        return val

    def _with_path(self, path: str) -> "GHPath":
        return GHPath(repo=self.repo, branch=self.branch, path=path.lstrip("/"), _info=self._info)
    ...
    def iterdir(self) -> Iterator["GHPath"]:
        if self.path:
            yield from (self._with_path(d["path"]) for d in self._info if d["path"].startswith(self.path))
        else:
            yield from (self._with_path(d["path"]) for d in self._info if "/" not in d["path"])
